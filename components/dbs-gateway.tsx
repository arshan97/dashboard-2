...
export function DBSGateway() {
  ...
  const handleToggle = (locationName: string, providerName: string, newState: boolean) => {
    setToggleDialog({
      isOpen: true,
  ...
  const submitToggleRequest = () => {
    ...
    toast({
      ...
      description: `Request to ${toggleDialog.newState ? "enable" : "disable"} ${toggleDialog.provider} in ${toggleDialog.location} has been submitted for approval.`,
    })
  ...
  return (
    <Card className="bg-card/50 backdrop-blur-sm shadow-lg">
      ...
      <CardContent className="px-2 py-2 space-y-4">
        {locations.map((location, index) => (
          <div key={location.name}>
            {index > 0 && <div className="border-t border-border/50 my-2"></div>}
            ...
            <div className="grid grid-cols-2 gap-2">
              {location.providers.map((provider) => (
                ...
                >
                  ...
                  <StatusIndicator status={provider.status} size="sm" className="mt-1 mb-1" />
                  <Switch
                    checked={provider.enabled}
                    onCheckedChange={(checked) => handleToggle(location.name, provider.name, checked)}
                    className="mt-1"
                  />
                </div>
              ))}
      ...
      <Dialog open={toggleDialog?.isOpen} onOpenChange={(open) => !open && setToggleDialog(null)}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Confirm Gateway Change Request</DialogTitle>
            <DialogDescription>
              Are you sure you want to request to {toggleDialog?.newState ? "enable" : "disable"}{" "}
