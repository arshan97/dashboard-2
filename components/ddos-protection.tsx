        ...
        <div className="space-y-4">
          {services.map((service) => (
            <div key={service.name} className="flex items-center justify-between p-2 border rounded">
              ...
              <Switch checked={service.enabled} onCheckedChange={() => handleToggle(service.name)} />
            </div>
          ))}
          <Button className="w-full" onClick={handleSubmit} disabled={pendingChanges.length === 0}>
            Submit Changes
          </Button>
        </div>
        ...
        <AlertDialog open={showConfirmation} onOpenChange={setShowConfirmation}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Are you sure you want to update the DDoS protection settings? This action cannot be undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel>Cancel</AlertDialogCancel>
              <AlertDialogAction onClick={confirmChanges}>Confirm</AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
      </CardContent>
